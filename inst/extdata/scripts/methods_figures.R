...
sample_profiles <- list(
  inf_hist = out$inf_history$inf_hist_mat[indivs$id, year_index],
  vac_hist = out$vac_history$vac_hist_mat[indivs$id, year_index],
  suscept_mat = out$inf_history$suscept_mat[indivs$id, year_index]
)
...
drift_dat_thinned <- drift_dat_thinned %>%
  add_row(Year = 2020,
          ...
          Gammas = drift_dat[drift_dat$Year == 2020,]$Gammas,
          Shade_Group = drift_dat[drift_dat$Year == 2020,]$Shade_Group + 1,
          Cum_Drift = drift_dat[drift_dat$Year == 2020,]$Cum_Drift,
          prev = 2018)

# plot
ymn <- min(drift_dat$Drift)
ymx <- max(drift_dat$Drift)

p_drift <- ggplot(data = drift_dat, mapping=aes( x=Year, y = Drift))+
  ...
  geom_rect(data = drift_dat_thinned,
            aes(xmin = prev, ymin = ymn, ymax = ymx, xmax = Year, fill = factor(Shade_Group),
                col = factor(Shade_Group)), alpha = 0.2) +
  geom_ribbon(aes(ymin = Drift,ymax = ymx), fill = "white", alpha = 1)+  # color shading under cumulative drift by shade group
  ylab('Antigenic Drift') +
...
p_drift <- p_drift + geom_point(data = drift_dat_thinned,aes(x=Year,y = Drift), colour = c(cols[-1],"#CC6666"))
p_drift
#################

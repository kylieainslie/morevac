...
#' @param timepoints Number of timepoints to simulate over. Defaults to 10.
...
#' @examples
#' initialize_pop()    
initialize_pop <- function(nindiv=1000,timepoints=10){
  # infection status matrix
    inf_stat_mat <- matrix(c(rep(NA, timepoints*nindiv)),nrow=nindiv)
    colnames(inf_stat_mat) <- c(paste0("AY",0:(timepoints-1)))
  # years since last infection matrix  
    x <- matrix(c(rep(999, timepoints*nindiv)),nrow=nindiv)
    colnames(x) <- c(paste0("AY",0:(timepoints-1)))
    
  # vaccination history matrix
    vac_hist_mat <- matrix(c(rep(NA, timepoints*nindiv)),nrow=nindiv)
    colnames(vac_hist_mat) <- c(paste0("AY",0:(timepoints-1)))
  # susceptibility matrix
    suscept_mat <- matrix(c(rep(1,timepoints*nindiv)),nrow=nindiv)
    colnames(suscept_mat) <- c(paste0("AY",0:(timepoints-1)))
  # ages
  # ages <- c(rep(0,nindiv))
    attack_rate <- c(rep(NA,timepoints))
    incidence <- c(rep(NA,timepoints))

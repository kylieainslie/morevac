### annual vaccination ###
# scenarios:
# 1. random children vaccinated each year
# 2. same children vaccinated each year
#setwd("~/Google Drive/morevac")
setwd("C:/Users/kainslie/Google Drive/morevac")
source("R/morevac_functions.R")

library(ggplot2)
### Scenario 1: random annual vaccination
run_annual1 <- function(n=100, years=5,vac_threshold=3,vac_coverage=0.5,
                        mybeta=0.15,delta_x=0.2,delta_v=0.3,mygamma=0.4,biannual=FALSE,
                        same_indiv = FALSE){
  ...
  for (year in 1:years){
     for(i in 1:n){
      ...
      # calculate susceptibility function for person i
        ...
        # never infected
        if (x[i,year] == 999){
          if (v[i,year] == 999 | v[i,year] > 1/delta_v){ # never vaccinated or vaccinated long enough ago for drift to have diminished protection
            suscept_mat[i,year] <- 1
          } else if (v[i,year]==0 | v[i,year]<=1/delta_v){ # vaccinated this year or within last few years 
            suscept_mat[i,year] <- (vac_hist_mat[i,year]*mygamma) + (v[i,year]*delta_v)
          } 
        }
        
        # infected and delta>0
        if (x[i,year]>=0 & x[i,year]<999 & delta_x>0 & delta_v>0){
          if (v[i,year]==999 | v[i,year] > 1/delta_v){ # never vaccinated
            if (x[i,year]< 1/delta_x){
              suscept_mat[i,year] <- x[i,year]*delta_x
            } else if (x[i,year]>= 1/delta_x) {
              suscept_mat[i,year] <- 1
            } 
          } else if (v[i,year]==0 | v[i,year]<=1/delta_v){ # vaccinated
            suscept_mat[i,year] <- min(x[i,year]*delta_x,(vac_hist_mat[i,year]*mygamma)+(v[i,year]*delta_v))
          } 
        }
